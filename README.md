...
**APP**: é a abreviação de application ou aplicativo. Trata-se de um programa desenvolvido para executar uma função específica, basicamente para o usuário.
...
**BIOS (Basic Input Output System)**: é um firmware que é executado quando ligamos um computador. Ela inicializa e testa o hardware, permitindo que o sistema operacional seja carregado. É uma interface entre o hardware e o sistema operacional, fornecendo serviços de tempo de execução e controlando a inicialização do computador.
...
**MAINFRAME**: é um computador de grande porte, com foco no processamento de um grande volume de informações.
...
**ROI (Return on Investment)**: é a mensuração do tempo necessário para retornar o valor investido na aquisição de novas soluções tecnológicas.

**SAAS (Software as a Service)**: é uma forma de comercialização de software onde o fornecedor se responsabiliza por toda a estrutura necessária para a disponibilização do sistema.
